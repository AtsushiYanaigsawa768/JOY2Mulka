...
| **早め・遅め** | 出走位置を前半／後半に寄せる。**個人でも所属ごとでも指定可** |
| **近め** | 指定した複数人をまとめて出走させる。**連続はせず、指定した分数以上は必ず空ける**。早め・遅めと重ねて指定可 |
| **ゼッケン番号** | `[コース][レーン][レーン内連番]` の桁設計。同一レーンが100人以上なら5桁に切り替え |
| **役職別スタートリスト** | 救護・スタート・フィニッシュ × 複数の並び順（計7種）を `.csv` と `.tex` で出力 |
| **多言語LaTeX** | 英語・日本語切り替え可能 |
| **ふりがな対応** | 役員用リストに読み仮名を表示 |

> 運営の手順そのものは [`docs/textbook/`](docs/textbook/) のテキストブックにまとめてあります
> （本編119ページ／練習会版11ページ／最小手順シート1枚）。
> 入出力ファイルのサンプルは [`sample/`](sample/README.md)。
