...
いずれも\textbf{UTF-8（BOM 付き）・改行 CRLF}で保存してあります。
\textbf{文字コードが違うと Mulka2 が認識しません}（Step 5）。
リレーはエントリー変更のたびに CSV を作り直すので，
\textbf{保存のたびに文字コードが変わっていないか}を見ます。
サンプルをコピーして中身を差し替えるのが確実です。
