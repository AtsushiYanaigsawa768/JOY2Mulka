...
`Mulka2 / JOY2Mulka による計算センター運営テキストブック`（日本語・A4・109 ページ）の
LaTeX ソースです。
...
| ファイル | 内容 |
|---|---|
| `main.pdf` | 本編・完成版 PDF（109 ページ） |
| `practice.pdf` | **練習会版**（22 ページ）。本編と同じ粒度で、練習会に必要な範囲だけを書いたもの |
...
| 第 6 部 | 逆引き索引 |
| 第 7 部 | 用語集＋ソフトウェア・資料の入手先 |

チェックリストの章（旧・第 6 部 CL-1〜CL-8）は削除した。
各 Step の `Done`（「できた」の判断基準）と、Step 37 の当日トラブル対応一覧、
巻末の逆引きが同じ役割を果たす。
...
**`normal/` と `relay/` の CSV は UTF-8（BOM 付き）・CRLF。**
Mulka2 は文字コードが違うと CSV を認識しない。
本文では Step 5 の `Key` ボックスで説明している。詳細は `sample/README.md`。

## ソフトウェアの入手先

第 7 部（用語集）の末尾に「ソフトウェア・資料の入手先」の表がある。
...
練習会・フリースタート専用の版（22 ページ）。
本編とは独立した自己完結の LaTeX ファイルで、プリアンブルも持っている。

- 想定：参加者 100 名以下／PC 1 台／スタート時刻を決めない／当日申込が中心
- **本編と同じ粒度**で書く。Step は P1〜P19 で、各 Step は
  `Goal` → 手順 → `Done` → `Fail` の 4 ブロックをそろえる
- 構成：やること 3 つ → 全 Step 早見表 → 前日まで（P1〜P7）→ 当日（P8〜P16）
  → 終わったあと（P17〜P19）→ トラブル初動カード → 逆引き
- タグは `\tALL` `\tSI` `\tEMIT` `\tSIAC`。Step の書式は `\Step[タグ]{タイトル}`
  （本編の `\Step{タイトル}{タグ}` とは引数の順が違う。タグ省略時は `\tALL`）
- JOY2Mulka の操作は本編と同じく `J2M` ボックスに置く（P6）
- チェックリストの節は削除した。各 Step の `Done` が同じ役割を果たす
